...
import { Plus, Pencil, Trash2, Search, Upload, X, Package, DollarSign, TrendingUp } from 'lucide-react';
...
export function MerchantProductManagement() {
  const { user } = useAuth();
  ...
  // Load products and categories from API (filtered by merchant)
  useEffect(() => {
    loadProducts();
    loadCategories();
  }, [user]);
  ...
  const loadProducts = async () => {
    ...
    try {
      setIsLoading(true);
      // Load all products and filter by merchant (or use a merchant filter if backend supports it)
      const response = await productsAPI.getAll({ limit: 1000 });
      const allProducts = response.data.data || response.data || [];
      const productsArray = Array.isArray(allProducts) ? allProducts : [];
      
      // Filter products by merchant ID if user is a merchant
      if (user.role === 'merchant') {
        const merchantProducts = productsArray.filter(
          (p) => p.merchant?._id?.toString() === user._id?.toString() || 
                 p.merchant?.toString() === user._id?.toString() ||
                 p.merchant === user._id
        );
  ...
  const totalRevenue = products.reduce((acc, p) => acc + (p.price || 0) * (p.stock > 0 ? 1 : 0), 0);
  ...
  const openAddDialog = (e) => {
    ...
    try {
      ...
      toast.error('Failed to open add product dialog');
    }
  };
